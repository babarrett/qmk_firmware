...
with open(input_filepath, "r") as read_file:
    ...
    with open("keyboard.part.0", "w") as write_file:
    ...
    for pseudolayer in data["pseudolayers"]:
        ...
        for chord in pseudolayer["chords"]:
            if chord["type"] == "chord_set":
                ...
                [output_buffer, number_of_chords, number_of_strings, strings] = add_chord_set(name, keycodes, chord["set"], data, output_buffer, number_of_chords, number_of_strings, strings)
            if chord["type"] == "visual_array":
                [output_buffer, number_of_chords, number_of_strings, strings] = add_dictionary(name, chord["keys"], chord["dictionary"], output_buffer, number_of_chords, number_of_strings, strings)
            if chord["type"] == "visual":
                keycodes = reduce(comma_separator, [word for word in chord["chord"]])
                [output_buffer, number_of_chords, number_of_strings, strings] = secret_chord(name, chord["keycode"], keycodes, data, output_buffer, number_of_chords, number_of_strings, strings)
            elif chord["type"] == "simple":
                keycodes = reduce(string_sum, ["H_" + word for word in chord["chord"]])
                [output_buffer, number_of_chords, number_of_strings, strings] = add_key(name, keycodes, chord["keycode"], output_buffer, number_of_chords, number_of_strings, strings)
    output_buffer += "\n"
    ...
    with open("keyboard.part.2", "w") as write_file:
        write_file.write(output_buffer)
    ...
    for string, i in zip(strings, range(0, len(strings))):
        output_buffer += "const char string_" + str(i) + " [] PROGMEM = \"" + string + "\";\n"

    output_buffer += "\n"
    output_buffer += "const char * const strings[] PROGMEM = {\n"
    if len(strings) > 0:
        output_buffer += "    " + reduce(comma_separator, ["string_" + str(i) for i in range(0, len(strings))]) 
    output_buffer += "\n};\n"

    
    with open("keyboard.part.1", "w") as write_file:
        write_file.write(output_buffer)
    ...
    with open("keyboard.part.0", "r") as file:
    ...
    with open("keyboard.part.1", "r") as file:
        output_buffer += file.read()
    output_buffer += "\n"
    ...
    with open("keyboard.part.2", "r") as file:
        output_buffer += file.read()
